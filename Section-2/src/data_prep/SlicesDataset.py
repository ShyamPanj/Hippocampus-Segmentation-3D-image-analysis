...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #sample["image"]=torch.tensor(img.reshape((1,img.shape[0],img.shape[1])))
        #sample["image"] = torch.from_numpy(self.data[min(slc[0],35)]["image"][slc[1]]).type(torch.cuda.FloatTensor).unsqueeze(0)
        #sample["seg"] = torch.from_numpy(self.data[min(slc[0],35)]["seg"][slc[1]]).type(torch.cuda.FloatTensor).unsqueeze(0)
        #sample["seg"]=torch.tensor(lbl.reshape((1,img.shape[0],img.shape[1])))
    ...
    def disply_slice(self,idx):
        print (self.slices[idx])
