...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    nd_img=nd_img.reshape((len(nd_img),1))
    # This is how you create a PIL image from numpy array
    pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize((512,512))
    # Paste the PIL image into our main report image object (pimg)
    pimg.paste(pil_i, box=(60,600))
...
def get_series_for_inference(path):
    ...
    path_series = [dir for dir, subdir, file in os.walk(path) if 'HCropVolume' in dir]
    print(path_series)
    rnd_path = np.random.choice(path_series)  
    series_for_inference = [pydicom.dcmread(os.path.join(rnd_path, f)) for f in os.listdir(rnd_path)]
...
if __name__ == "__main__":
    ...
    # STA UT SUGGESTION: save_report_as_dcm has some suggestions if you want to expand your
    ...
    os_command("sudo storescu localhost 4242 -v -aec HIPPOAI +r +sd /home/workspace/out/report.dcm")
